...
Pointer<T,size>::Pointer(T *t){
    ...
    typename std::list<PtrDetails<T> >::iterator itr = findPtrInfo(t);
...
Pointer<T,size>::Pointer(const Pointer &ob){
    ...
    // TODO: Implement Pointer constructor
    // Lab: Smart Pointer Project Lab
    typename std::list<PtrDetails<T> >::iterator itr = findPtrInfo(ob.addr);
    itr -> refcount++;
    addr = ob.addr;
    isArray = ob.isArray;
...
Pointer<T, size>::~Pointer(){
    ...
    typename std::list<PtrDetails<T> >::iterator itr = findPtrInfo(addr);
    if (itr -> refcount) itr -> refcount--;
    collect();
    return;
}
...
bool Pointer<T, size>::collect(){
    ...
    do {
        for (itr = refContainer.begin(); itr != refContainer.end(); itr++){
            ...
            freed = true;
            if (itr -> isArray) delete[] itr -> memPtr;
            else delete itr -> memPtr;
